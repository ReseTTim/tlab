<template>
  <form 
    ...
    <fieldset>
      <legend class="sr-only">{{ $t('search.legend') }}</legend>
      <div class="form-group">
        ...
        <button type="submit" name="submit" :aria-label="$t('search.button')"
          class="btn">
          <svg class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.879 119.799" enable-background="new 0 0 122.879 119.799" xml:space="preserve">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.ui.setErrorMessage(this.$t('search.error'));
    },
    onShortcut(event) {
...
<style lang="scss" scoped>
  .search {
    --search-color: var(--color-11);

    border-radius: var(--default-radius);
    border: var(--border-size-1) solid var(--search-color);
    display: inline-flex;

    &:hover,
    &:focus,
    &:has(input:focus) {
      --search-color: var(--color-10);
